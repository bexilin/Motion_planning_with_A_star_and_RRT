...
def RRT_connect(start, env, robot):
    ...
    while 1:
        ...
        if 0 <= distance <= 0.05:
            ...
            # compute input set
            phidotset = [-pi / 10, -pi / 15, -pi / 30, 0., pi / 30, pi / 15, pi / 10]
            udotset = [-0.75, -0.5, -0.25, 0., 0.25, 0.5, 0.75]
            ...
            while 1:
                ...
                primitive = []
                for i in inputset:
                    if (closest.x-(-8.))*(closest.x-(-14)) < 0 and (closest.y-14.5)*(closest.y-17.5) < 0 and i[1] != -0.75 :
                        continue
